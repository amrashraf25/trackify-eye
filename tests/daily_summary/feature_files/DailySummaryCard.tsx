...
/**
 * Returns the human-readable label for a behavior type.
 * Falls back to the raw type string (underscores replaced with spaces).
 */
export function getBehaviorLabel(type: string): string {
  return BEHAVIOR_META[type]?.label ?? type.replace(/_/g, " ");
}

/**
 * Returns the Tailwind CSS dot-color class for a severity level.
 * Returns empty string for unknown severities.
 */
...
/**
 * Returns the key (type name) of the most-common behavior in by_type.
 * Returns null when the map is empty.
 */
export function getTopBehavior(by_type: Record<string, number>): string | null {
...
/**
 * Returns behavior type entries sorted descending by count (highest first).
 */
export function sortBehaviorsByCount(by_type: Record<string, number>): [string, number][] {
  return Object.entries(by_type).sort((a, b) => b[1] - a[1]);
}
