...
class FastFaceRecognition:
    ...
    def load_students_from_folder(self):
        if not os.path.exists(self.students_folder): return
        valid_extensions = {'.jpg', '.jpeg', '.png', '.bmp'}
        student_files = [f for f in os.listdir(self.students_folder) if any(f.lower().endswith(ext) for ext in valid_extensions)]
        loaded_count = 0
    ...
    def extract_face_embedding(self, image):
        if self.face_app is None: return None
        try:
    ...
    def recognize_face_fast(self, frame, face_rect):
        if self.face_app is None or len(self.student_embeddings) == 0: return "Unknown", 0.0
        x1, y1, x2, y2 = face_rect
        try:
            face_region = frame[y1:y2, x1:x2]
            if face_region.size == 0: return "Unknown", 0.0
            ...
            if best_similarity > 0.4: return self.student_names[best_match_idx], best_similarity
            else: return "Unknown", best_similarity
        except Exception: return "Unknown", 0.0

    def detect_eyes_closed(self, face_landmarks):
        if face_landmarks is None: return False
    ...
    def detect_behaviors(self, face_data, yolo_objects):
        ...
        behaviors = []
        if self.detect_talking(face_data['landmarks']): behaviors.append("Talking")
        if self.detect_eyes_closed(face_data['landmarks']):
            self.eye_closed_frames += 1
            if self.eye_closed_frames >= self.sleeping_threshold: behaviors.append("Sleeping")
        else: self.eye_closed_frames = 0
        for obj in yolo_objects:
            label = obj['label']
            if "cell" in label or "phone" in label: behaviors.append("Phone")
            if "bottle" in label or "cup" in label: behaviors.append("Drinking")
            if label in ["apple", "banana", "sandwich"]: behaviors.append("Eating")
        ...
        while True:
            ...
            for i, face_data in enumerate(faces):
                ...
                for obj in yolo_objects:
                    ...
                    if overlap: face_specific_objects.append(obj)
                ...
                else: student_name, similarity = self.last_recognition.get(i, ("Unknown", 0.0))
                ...
                for behavior in behaviors:
                    ...
                    if self.should_report(report_key):
                        if behavior == "Phone": report_phone_detected(self.room_number, student_name)
                        else:
                ...
                cv2.putText(frame, behavior_text, (x1, y2+20), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 0), 2)
            cv2.putText(frame, f"FPS: {current_fps}", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)
    ...
    <div className="glass rounded-2xl overflow-hidden">
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div>
            <h4 className="font-bold text-foreground text-sm">Python Integration Script</h4>
            <p className="text-[10px] text-muted-foreground">Copy & run: <code className="font-mono bg-secondary/50 px-1.5 py-0.5 rounded">python trackify.py</code></p>
          </div>
        </div>
      ...
      {isExpanded && (
        <div className="border-t border-border/30">
          <pre className="p-4 overflow-x-auto text-xs bg-secondary/20 max-h-96 overflow-y-auto">
