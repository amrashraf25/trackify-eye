// Donut pie chart showing the distribution of student behavior scores on the dashboard.
...
const BehaviorPieChart = () => {
  ...
  // Fetch all behavior scores and group them into Excellent / Good / Needs Work buckets
  const { data: chartData = [] } = useQuery({
    queryKey: ["dashboard-behavior-pie"],
    queryFn: async () => {
      const { data: scores } = await supabase.from("behavior_scores").select("score");
      // Return zero-value placeholders when there are no records yet
      ...
      // Count students per tier based on their numeric score
      ...
      // Convert raw counts to percentage share for each tier
