// Horizontal progress-bar chart showing the breakdown of grade types (exam, quiz, etc.) on the dashboard.
...
const GradesComposition = () => {
  ...
  // Fetch grade_type values and compute each type's share of total grades
  const { data: gradeData = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      // Aggregate count per grade type
      ...
      // Predefined color mapping for known grade types
      ...
      // Cycled fallback colors for any unknown grade types
      const fallbackColors = ["bg-primary", "bg-neon-cyan", "bg-emerald-500", "bg-amber-500"];

      // Sort types descending by count and map to display-ready objects
  ...
  // Show a neutral placeholder row when no grade data exists yet
