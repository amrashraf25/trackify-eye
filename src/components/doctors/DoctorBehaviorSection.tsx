// Displays, filters, and manages behavior records and scores for a single doctor, with a week heatmap and per-course breakdown.
...
// Semester spans 16 teaching weeks
...
const DoctorBehaviorSection = ({ doctorId, doctorName, userId, doctorCourses }: DoctorBehaviorSectionProps) => {
  ...
  // Load behavior records; week filter is applied server-side, course filter client-side
  const { data: records = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      // course_id filtering is handled client-side after the query
  ...
  // Apply course filter client-side since the API doesn't support it server-side yet
  const filteredRecords = selectedCourseId !== "all"
    ? records.filter((r: any) => r.course_id === selectedCourseId)
    : records;

  // Compute the effective score by summing all score_change values from 100; clamp between 0–100
  ...
  // Insert a new behavior record; invalidates both records and aggregate score queries on success
  ...
  // Delete records scoped to the active course filter (or all records) and reset the stored score
  const handleReset = async () => {
    if (selectedCourseId !== "all") {
      // Scope the delete to the currently selected course only
      await (supabase.from("doctor_behavior_records").delete().eq("doctor_id", doctorId) as any).eq("course_id", selectedCourseId);
    } else {
      // Delete all records and explicitly reset the aggregate score back to 100
